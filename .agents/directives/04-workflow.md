...
- `.husky/commit-msg`의 version lifecycle gate는 `feat`·`fix`·`perf`·`revert`의 patch +1, major·minor release marker, schema source 변경의 새 migration SQL, migration filename·DB marker와 앱 version 일치를 강제한다. `--no-verify` 우회 금지.
