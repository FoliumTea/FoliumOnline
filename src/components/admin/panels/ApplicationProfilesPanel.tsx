                    ...
                    {profiles.map((profile) => (
                        ...
                        >
                            ...
                            <p className="mt-1 text-xs text-(--color-muted)">
                                {profile.parent_job_field} · {profile.status}
                                {profile.needsUpdate ? " · 업데이트 가능" : ""}
                            </p>
                        </button>
                    ))}
                </div>
            ...
            <section className="min-w-0 flex-1 overflow-y-auto rounded-2xl border border-(--color-border) bg-(--color-surface) p-6">
                ...
                <div className="mt-6 grid gap-5">
                    ...
                    <label className="grid gap-2 text-sm font-semibold">
                        제외할 포트폴리오
                        <input
                            value={formatList(overrides.excludedPortfolioSlugs)}
                            onChange={(event) =>
                                setOverrides((current) => ({
                                    ...current,
                ...
                <div className="mt-7 flex flex-wrap gap-3">
                    ...
                    {selected && (
                        <button
                            type="button"
                            onClick={() =>
                                void runProfileAction(publishApplicationProfile)
                            }
                            disabled={saving}
                            className="rounded-lg border border-(--color-border) px-4 py-2 font-bold"
                        >
                    ...
                    {selected?.status === "published" && (
                        <button
                            type="button"
                            onClick={() =>
                                void runProfileAction(revokeApplicationProfile)
                            }
                            disabled={saving}
